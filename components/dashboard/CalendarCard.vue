  ...
  <BaseCard>
    <template #header>
      <div class="flex items-center justify-between w-full">
        <div>
          ...
          <p class="text-[11px] text-gray-500 mt-0.5">{{ calendar.year }}</p>
        </div>
        <div class="flex items-center gap-0.5">
          <button class="h-6 w-6 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors">
    ...
    <div class="grid grid-cols-7 text-center">
      <div
        ...
        :key="d"
        class="py-1.5 text-[9px] label-mono text-gray-400 select-none"
      >
        {{ d }}
      </div>
      <div v-for="day in days" :key="day" class="py-[3px]">
        <button
          :class="[
            'relative h-7 w-7 mx-auto rounded text-[11px] flex items-center justify-center transition-colors',
            day === calendar.today
              ? 'metallic-accent text-white font-semibold'
              : day < calendar.today
                ? 'text-gray-300'
    ...
    <div>
      <p class="text-[9px] label-mono text-gray-400 mb-2.5 select-none">Upcoming</p>
      <div class="space-y-1">
        <div
          v-for="(bill, i) in calendar.upcoming"
          :key="bill.name"
          class="flex items-center gap-2.5 py-1 px-1.5 -mx-1.5 rounded hover:bg-gray-50 transition-colors"
        >
          ...
          <span class="text-[10px] text-gray-500 tabular-nums">Feb {{ bill.day }}</span>
